...
class Normal(
    ...
    def allow_dependent_rows(self, operator):
        ...
        if rows > columns:  # strictly tall
            ...
            inner_operator = conj(operator.transpose()) @ operator
            # WARNING to custom inner solver implementers: In this case it is
            # impossible to infer from assumptions by the inner solver if the
            # row space of operator should be assumed constant.

            # To be completely safe, one should return True here to support the
            # possibility that operator has nonconstant row space, constant
            # column space (necessarily non full rank), and the inner solver
            # detects this situation and reports False for both
            # allow_dependent_{rows,columns}.

            # In the most common case (e.g., for every solver implemented in
            # lineax), constant row space means full rank and not more rows
            # than columns. A necessary condition to be in this case is that
            # the inner solver assumes both the row space and column space is
            # constant. To avoid emitting an inefficient jvp term for the
            # lineax inner solvers, we attempt to detect this common case by
            # checking this necessary condition.
            inner_solver_assumes_constant_row_and_col_space = (
                not self.inner_solver.allow_dependent_columns(inner_operator)
                and not self.inner_solver.allow_dependent_rows(inner_operator)
            )
            return not inner_solver_assumes_constant_row_and_col_space
